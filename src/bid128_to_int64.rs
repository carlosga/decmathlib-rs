...
use crate::bid128::{bid_maskhigh128, bid_midpoint128, bid_midpoint64, bid_nr_digits, bid_shiftright128, bid_ten2k64, bid_ten2mk128, bid_ten2mk128trunc};
    ...
    if ((q + exp) > 19) { // x >= 10^19 ~= 2^63.11... (cannot fit in BID_SINT64)
        ...
        *pfpsf |= StatusFlags::BID_INVALID_EXCEPTION;
        ...
        if (x_sign) { // if n < 0 and q + exp = 19
            ...
            if (q <= 19) { // 1 <= q <= 19 => 1 <= 20-q <= 19 =>
                // 10^(20-q) is 64-bit, and so is C1
                __mul_64x64_to_128MACH(C1, C1.w[0], bid_ten2k64[(20 - q) as usize]);
            } else if (q == 20) {
                ; // C1 * 10^0 = C1
            } else { // if 21 <= q <= 34
                __mul_128x64_to_128(C, bid_ten2k64[(q - 20) as usize], C); // max 47-bit x 67-bit
            }
            if (C1.w[1] > C.w[1] || (C1.w[1] == C.w[1] && C1.w[0] > C.w[0])) {
                // set invalid flag
                *pfpsf |= StatusFlags::BID_INVALID_EXCEPTION;
