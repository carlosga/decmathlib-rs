...
use crate::d128::bid_internal::{__mul_128x128_to_256, __mul_64x64_to_128MACH};
use crate::d128::dec128::{BID_UINT128, BID_UINT256, BID_UINT64};
...
pub (crate) fn bid_round128_19_38(
    ...
    let mut ptr_Cstar: BID_UINT128 = BID_UINT128::default();
...
void bid_round192_39_57 (int q,
  ...
  ind = q - x;	// 1 <= ind <= q - 1
  if (ind <= 19) {
    if (Cstar.w[2] == 0x0u64 && Cstar.w[1] == 0x0u64 &&
    ...
    // if ind = 20
    if (Cstar.w[2] == 0x0u64 && Cstar.w[1] == bid_ten2k128[0].w[1] &&
    ...
    if (Cstar.w[2] == 0x0u64 && Cstar.w[1] == bid_ten2k128[ind - 20].w[1] &&
...
void bid_round256_58_76 (int q,
  ...
  ind = q - x;	// 1 <= ind <= q - 1
  if (ind <= 19) {
    if (Cstar.w[3] == 0x0u64 && Cstar.w[2] == 0x0u64 &&
	Cstar.w[1] == 0x0u64 && Cstar.w[0] == bid_ten2k64[ind]) {
      // if  Cstar = 10^(q-x)
      Cstar.w[0] = bid_ten2k64[ind - 1];	// Cstar = 10^(q-x-1)
      *incr_exp = 1;
    } else {
    ...
    if (Cstar.w[3] == 0x0u64 && Cstar.w[2] == bid_ten2k256[0].w[2] &&
    ...
    if (Cstar.w[3] == 0x0u64 && Cstar.w[2] == bid_ten2k256[ind - 39].w[2] &&
	Cstar.w[1] == bid_ten2k256[ind - 39].w[1] &&
	Cstar.w[0] == bid_ten2k256[ind - 39].w[0]) {
      // if  Cstar = 10^(q-x)
      Cstar.w[0] = bid_ten2k256[ind - 40].w[0];	// Cstar = 10^(q-x-1)
