...
use crate::bid_binarydecimal::{binary32_to_bid128, binary64_to_bid128};
...
impl d128 {
    ...
    /// Convert 32-bit binary floating-point value to 128-bit decimal
    ...
    /// Convert 64-bit binary floating-point value to 128-bit decimal
    /// floating-point format (binary encoding)
    #[must_use]
    pub fn convert_from_f64(value: f64, rnd_mode: Option<RoundingMode>, pfpsf: &mut _IDEC_flags) -> Self {
        binary64_to_bid128(value, rnd_mode.unwrap_or(DEFAULT_ROUNDING_MODE), pfpsf)
    }
...
impl From<f64> for d128 {
    ...
    fn from(value: f64) -> Self {
        let mut status: _IDEC_flags = 0;
        binary64_to_bid128(value, DEFAULT_ROUNDING_MODE, &mut status)
    }
}
