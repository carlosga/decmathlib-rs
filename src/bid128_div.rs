...
use crate::bid_convert_data::{bid_factors, bid_packed_10000_zeros};
use crate::bid_decimal_data::*;
use crate::bid_div_macros::{bid___div_128_by_128, bid___div_256_by_128};
use crate::bid_internal::{__mul_128x128_full, __mul_128x128_low, __mul_128x128_to_256, __mul_64x128_short, __mul_64x64_to_128, __set_status_flags, __shr_128, __shr_128_long, __sub_borrow_out, __unsigned_compare_ge_128, __unsigned_compare_gt_128, bid_get_BID128};
...
pub (crate) fn bid128_div(x: &BID_UINT128, y: &BID_UINT128, rnd_mode: u32, pfpsf: &mut _IDEC_flags) -> BID_UINT128 {
    ...
    let mut T: BID_UINT64;
    let mut carry64: BID_UINT64;
    let mut D: BID_UINT64;
    let mut Q_high: BID_UINT64;
    let mut Q_low: BID_UINT64;
    let mut QX: BID_UINT64;
    let mut PD: BID_UINT64;
    ...
    if (!CA4.w[0] && !CA4.w[1]) {   // check whether result is exact
        ...
        if (!CX.w[1] && !CY.w[1] && (CX.w[0] <= 1024) && (CY.w[0] <= 1024)) {
            ...
            if (!Q_low) {
                ...
                if (!digit && !tdigit[1]) {
                    ...
                    if (!(digit_h & 1)) {
                        nzeros += 3 & ((bid_packed_10000_zeros[digit_h >> 3] >> (digit_h & 7)) as BID_UINT32);
                    }
                }
    ...
    // #else
        rmode = rnd_mode;
        if ((sign_x ^ sign_y) != 0 && ((rmode - 1) as u32) < 2) {
            rmode = 3 - rmode;
        }
        match rmode {
            RoundingMode::BID_ROUNDING_TO_NEAREST =>  { // round to nearest code
                ...
                CA4r.w[1] = CA4r.w[1] - CY.w[1] - carry64;
                D         = (CA4r.w[1] | CA4r.w[0]) ? 1 : 0;
                carry64   = ((1 + (((CA4r.w[1] as BID_SINT64) >> 63)) & ((CQ.w[0]) | D)) as BID_UINT64);
                CQ.w[0]  += carry64;
                if (CQ.w[0] < carry64) {
                    CQ.w[1] += 1;
                }
            ...
            RoundingMode::BID_ROUNDING_TIES_AWAY => {
                ...
                CA4r.w[1] = CA4r.w[1] - CY.w[1] - carry64;
                D         = if (CA4r.w[1] | CA4r.w[0]) { 0 } else { 1 };
                carry64   = ((1 + (((CA4r.w[1] as BID_SINT64)) >> 63)) | D) as BID_UINT64;
                CQ.w[0]  += carry64;
                if (CQ.w[0] < carry64) {
                    CQ.w[1] += 1;
                }
