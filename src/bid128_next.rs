        ...
        if x.w[1] == 0x5fffed09bead87c0u64 && x.w[0] == 0x378d8e63ffffffffu64 {
            ...
            if q1 < P34 {
                ...
                if exp + 6176 > P34 - q1 {
                    ...
                    x_exp = x_exp - ((ind as BID_UINT64) << 49);
                } else {
...
if (x.w[1] == 0xdfffed09bead87c0u64
&& x.w[0] == 0x378d8e63ffffffffu64) {
    // x = -MAXFP = -999...99 * 10^emax
    res.w[1] = 0xf800000000000000u64;	// -inf
    res.w[0] = 0x0000000000000000u64;
} else if (x.w[1] == 0x0u64 && x.w[0] == 0x0000000000000001u64) {	// +MINFP
...
if (C1.w[1] == 0) {
    if (C1.w[0] >= 0x0020000000000000u64) {	// x >= 2^53
        // split the 64-bit value in two 32-bit halves to avoid rnd errors
        if (C1.w[0] >= 0x0000000100000000u64) {	// x >= 2^32
    ...
    if (exp + 6176 > P34 - q1) {
        ...
        if (q1 <= 19) {	// 64-bit C1
            if (ind <= 19) {	// 64-bit 10^ind and 64-bit C1
                __mul_64x64_to_128MACH (C1, C1.w[0], bid_ten2k64[ind as usize]);
            } else {	// 128-bit 10^ind and 64-bit C1
                __mul_128x64_to_128 (C1, C1.w[0], bid_ten2k128[(ind - 20) as usize]);
            }
        } else {	// C1 is (most likely) 128-bit
            if (ind <= 14) {	// 64-bit 10^ind and 128-bit C1 (most likely)
                __mul_128x64_to_128 (C1, bid_ten2k64[ind as usize], C1);
...
if (C1.w[0] == 0xffffffffffffffffu64)
C1.w[1]--;
if (x_exp != 0 && C1.w[1] == 0x0000314dc6448d93u64 && C1.w[0] == 0x38c15b09ffffffffu64) {	// if  C1 = 10^33 - 1
    C1.w[1] = 0x0001ed09bead87c0u64;	// C1 = 10^34 - 1
    C1.w[0] = 0x378d8e63ffffffffu64;
    x_exp = x_exp - EXP_P1;
}
