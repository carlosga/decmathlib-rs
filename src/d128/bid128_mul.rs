...
use crate::d128::dec128::{bid64_to_bid128, BID_UINT128, BID_UINT64};

pub (crate) fn bid64dq_mul(x: &BID_UINT64, y: &BID_UINT128, rnd_mode: u32) -> BID_UINT64 {
    let x1: BID_UINT128 = bid64_to_bid128(*x);

    return bid64qq_mul(&x1, y, rnd_mode);
}

pub (crate) fn bid64qd_mul(x: &BID_UINT128, y: &BID_UINT64, rnd_mode: u32) -> BID_UINT64 {
    let y1: BID_UINT128 = bid64_to_bid128(*y);

    return bid64qq_mul(x, &y1, rnd_mode);
}

pub (crate) fn bid64qq_mul(x: &BID_UINT128, y: &BID_UINT128, rnd_mode: u32) -> BID_UINT64 {
...
pub (crate) fn bid128dd_mul(x: &BID_UINT64, y: &BID_UINT64, rnd_mode: u32) -> BID_UINT128 {
    ...
    let res = bid128_mul(&x1, &y1, rnd_mode);

    return res;
}

pub (crate) fn bid128dq_mul(x: &BID_UINT64, y: &BID_UINT128, rnd_mode: u32) -> BID_UINT128 {
    let x1  = bid64_to_bid128(*x);
    let res = bid128_mul(&x1, &y, rnd_mode);

    return res;
}

pub (crate) fn bid128qd_mul(x: &BID_UINT128, y: &BID_UINT64, rnd_mode: u32) -> BID_UINT128 {
  let y1  = bid64_to_bid128(*y);
  let res = bid128_mul(&x, &y1, rnd_mode);
  return res;
}

/// bid128_mul stands for bid128qq_mul
pub (crate) fn bid128_mul(x: &BID_UINT128, y: &BID_UINT128, rnd_mode: u32) -> BID_UINT128 {
    let mut z: BID_UINT128 = BID_UINT128 { w: [0x0000000000000000u64, 0x5ffe000000000000u64] };
